...
TRexServer::TRexServer(TRexEngine* parEngine, int parPort,
                       GenericRoutingTable* parRoutingTable, int parQueueSize) {
  engine = parEngine;
  port = parPort;
  ...
  pthread_create(writingThread, NULL, trex_server::startSendingThread,
                 (void*)this);
  pthread_create(inputReaderThread, NULL, trex_server::startQueueReader,
                 (void*)this);
...
void TRexServer::stop() { running = false; }
...
void TRexServer::writeToInputQueue(int clientId) {
  ...
  while (running) {
    ...
    if (pktType == PUB_PKT) {
      ...
      cout << "Received an unknown packet, probably due to a communication "
              "error: disconnecting the client." << endl;
      removeDisconnectedClient(clientId, true);
    }
  }
}
